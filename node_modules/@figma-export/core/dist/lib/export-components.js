...
exports.components = ({ token, fileId, onlyFromPages = [], transformers = [], outputters = [], log = (msg) => {
    // eslint-disable-next-line no-console
    console.log(msg);
}, }) => __awaiter(void 0, void 0, void 0, function* () {
    const client = figma_1.getClient(token);
    log('fetching document');
    const { data: { document } = {} } = yield client.file(fileId);
    if (!document) {
        throw new Error('\'document\' is missing.');
    }
    const pages = figma_1.getPages((document), { only: onlyFromPages });
    log('fetching components');
    const pagesWithSvg = yield figma_1.enrichPagesWithSvg(client, fileId, pages, transformers);
